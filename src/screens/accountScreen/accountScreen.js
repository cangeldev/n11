    ...
    <View style={style.container}>
      <ScrollView>
        ...
        <View style={style.listView}>
          {
            list.slice(0, 10).map((item) => <AccountScreenButton key={item.key} category={item} />)
          }
        </View>
        <Text style={style.title}>Popüler Sayfalar</Text>
        <View style={style.listView}>
          {
            list.slice(10, 17).map((item) => <AccountScreenButton key={item.key} category={item} />)
          }
        </View>
        <Text style={style.title}>Diğer</Text>
        <View style={[style.listView, { marginBottom: 15 }]}>
          {
            list.slice(17, 22).map((item) => <AccountScreenButton key={item.key} category={item} />)
          }
        </View>
      </ScrollView>
    </View>
